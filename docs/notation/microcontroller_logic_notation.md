...
- **Textual over graphical**: Uses simple ASCII characters (lower 127 chars) within markdown code blocks
...
- - -    Clock signal
~~~~~    Analog signal
^v       Edge-triggered (^=rising, v=falling)
#        High level
.        Low level
<>       Tri-state/floating
...
Directions:     <->^v (arrows) ====> (bus direction)
