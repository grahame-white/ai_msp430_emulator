...
/// </summary>
public class AndInstruction : Instruction, IExecutableInstruction
{
    /// <summary>
    ...
    /// <summary>
    /// Executes the AND instruction on the specified CPU state.
    /// Performs a bitwise AND operation between source and destination operands and updates the N, Z flags.
    ...
    public uint Execute(IRegisterFile registerFile, byte[] memory, ushort[] extensionWords)
    {
        ...
        // Perform AND operation: dst = src & dst
        ...
        // Update flags: AND instruction affects N, Z; clears C, V
        ...
        registerFile.StatusRegister.Carry = false;  // AND always clears carry
        registerFile.StatusRegister.Overflow = false;  // AND always clears overflow
